...
from cvxpy.reductions.cone2cone.affine2direct import Dualize
from cvxpy.reductions.cone2cone.affine2direct import Slacks
...
def vectorized_lower_tri_to_triples(A: sp.sparse.coo_matrix | list[float] | np.ndarray, dim: int) -> tuple[
...
class MOSEK(ConicSolver):
		...
		if 'dualized' in data:
			if len(data[s.C]) == 0 and len(data['c_bar_data']) == 0:
				...
				if np.linalg.norm(data[s.B]) > 0:
					sol = Solution(s.INFEASIBLE, -np.inf, dict(), {s.EQ_DUAL: data[s.B]}, dict())
					return {'sol': sol}
				else:
	...
	def invert(self, solver_output, inverse_data):
		...
		STATUS_MAP = defaultdict(lambda: s.INFEASIBLE, STATUS_MAP)
	...
	def recover_dual_variables(task, sol):
		...
		if task.getnumintvar() == 0:
			...
			dual_vars[s.EQ_DUAL] = np.array(
				dual_var)  # We only need to recover dual variables related to the equality constraints.  # Dual variables related to the "direct" conic constraints are not needed when  # inverting the solution to undo dualization.
		else:
			dual_vars = None
		return dual_vars
	...
	def handle_options(env, task, verbose: bool, solver_opts):
		...
		if 'mosek_params' in kwargs:
			...
			if any(
				isinstance(param, mosek.iparam) or isinstance(param, mosek.dparam) or isinstance(param, mosek.sparam) for param
				in solver_opts['mosek_params'].keys()):
