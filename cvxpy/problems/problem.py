...
class Problem(u.Canonical):
	...
	def get_problem_data(self, solver, gp: bool = False, enforce_dpp: bool = False, ignore_dpp: bool = False,
		verbose: bool = False, canon_backend: str | None = None, solver_opts: Optional[dict] = None):
		...
		if key != self._cache.key:
			...
			solving_chain = self._construct_chain(solver=solver, gp=gp, enforce_dpp=enforce_dpp, ignore_dpp=ignore_dpp,
		...
		if self._cache.param_prog is not None:
			...
			if verbose:
				s.LOGGER.info('Using cached ASA map, for faster compilation '
			...
			safe_to_cache = (isinstance(data, dict) and s.PARAM_PROB in data and not any(
			...
			if safe_to_cache:
				if verbose and self.parameters():
					s.LOGGER.info('(Subsequent compilations of this problem, using the '
	...
	def _find_candidate_solvers(self, solver=None, gp: bool = False):
		...
		if gp:
			if solver is not None and solver not in slv_def.CONIC_SOLVERS:
				raise error.SolverError("When `gp=True`, `solver` must be a conic solver "
				                        "(received '%s'); try calling " % solver + " `solve()` with `solver=cvxpy.ECOS`.")
			elif solver is None:
		...
		if self.is_mixed_integer():
			...
			if not candidates['conic_solvers'] and not candidates['qp_solvers']:
				raise error.SolverError("Problem is mixed-integer, but candidate "
	...
	def _construct_chain(self, solver: Optional[str] = None, gp: bool = False, enforce_dpp: bool = False,
		ignore_dpp: bool = False, canon_backend: str | None = None, solver_opts: Optional[dict] = None) -> SolvingChain:
		"""
		...
		return construct_solving_chain(self, candidate_solvers, gp=gp, enforce_dpp=enforce_dpp, ignore_dpp=ignore_dpp,
		                               canon_backend=canon_backend, solver_opts=solver_opts)
	...
	def _solve(self, solver: str = None, warm_start: bool = True, verbose: bool = False, gp: bool = False,
	           qcp: bool = False, requires_grad: bool = False, enforce_dpp: bool = False, ignore_dpp: bool = False,
	           canon_backend: str | None = None, **kwargs):
		...
		for parameter in self.parameters():
			if parameter.value is None:
				raise error.ParameterError("A Parameter (whose name is '%s') does not have a value "
		...
		if verbose:
			...
			s.LOGGER.info('Your problem has %d variables, '
			              '%d constraints, and ' '%d parameters.', n_variables, len(self.constraints), n_parameters)
			curvatures = []
			if self.is_dcp():
			...
			if n_parameters == 0:
				s.LOGGER.info('(If you need to solve this problem multiple times, '
				              'but with different data, consider using parameters.)')
			s.LOGGER.info('CVXPY will first compile your problem; then, it will '
		...
		if requires_grad:
			...
			if qcp:
				...
				raise ModuleNotFoundError("The Python package diffcp must be installed to "
			...
			if qcp and not self.is_dcp():
				...
				if verbose:
					s.LOGGER.info('Reducing DQCP problem to a one-parameter '
		...
		data, solving_chain, inverse_data = self.get_problem_data(solver, gp, enforce_dpp, ignore_dpp, verbose,
		...
		if verbose:
			...
			s.LOGGER.info('Solver (including time spent in interface) took '
	...
	def unpack_results(self, solution, chain: SolvingChain, inverse_data) -> None:
		...
		if solution.status in s.INACCURATE:
			warnings.warn("Solution may be inaccurate. Try another solver, "
		...
		if solution.status in s.ERROR:
			raise error.SolverError(
				"Solver '%s' failed. " % chain.solver.name() + "Try another solver, or solve with verbose=True for more "
				                                               "information.")
		
		self.unpack(solution)
		self._solver_stats = SolverStats(self._solution.attr, chain.solver.name())
